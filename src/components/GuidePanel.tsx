...
// The interactive Storyteller guide. It drives the whole game: setup → each
// night (in order, highlighting who to wake) → each day, using the shared
// stepIndex. Prev/Next moves within a phase and rolls over into the next.
export function GuidePanel(props: Props) {
  ...
  if (s.phase.type === "setup")
    return (
      <SetupGuide
        ...
        onRevealAll={props.onRevealAll}
      />
    );

  if (s.phase.type === "night") {
    ...
    const fakes = (step?.seatIds ?? [])
      ...
      .map((x) => x.name);
    return (
      <div className="guide guide--night">
        <div className="guide__top">
          <span className="guide__phase">Night {s.phase.count}</span>
          <span className="guide__count">
            Step {idx + 1} / {nightSteps.length}
          </span>
        </div>
        <div className="guide__body">
          ...
          {impaired.length > 0 && (
            <p className="guide__warn">
              ⚠ {impaired.join(", ")} — their ability malfunctions. Act as if it
              doesn't work (give false or no information).
            </p>
          )}
          {fakes.length > 0 && (
            <p className="guide__warn">
              🎭 {fakes.join(", ")} aren't really this character (Drunk/Lunatic). Wake
              them to keep up the act, but their ability does nothing.
            </p>
          )}
        ...
        <div className="guide__nav">
          ...
          {atEnd ? (
            <button
              ...
              onClick={() => goPhase({ type: "day", count: s.phase.count })}
            >
  ...
  return (
    <div className="guide guide--day">
      <div className="guide__top">
        <span className="guide__phase">Day {s.phase.count}</span>
        <span className="guide__count">
          Step {idx + 1} / {DAY_STEPS.length}
        </span>
      </div>
      <div className="guide__body">
          ...
          <p className="guide__who">
            {executed ? (
              ...
              <span className="muted">
                Execution not confirmed. Close a vote to execute, or mark no execution.
              </span>
            )}
          </p>
        )}
      ...
      <div className="guide__nav">
        ...
        {atEnd ? (
          <button
            className="pbtn primary-btn"
            onClick={() => goPhase({ type: "night", count: s.phase.count + 1 })}
          >
...
function SetupGuide({
  ...
  state: HostState;
  goPhase: (p: HostState["phase"]) => void;
  canAutoFill: boolean;
  onAutoFill: () => void;
  onRevealAll: () => void;
}) {
  ...
  const done = [
    ...
    allAssigned,
    bluffs === 3,
    revealed === s.seats.length && revealed > 0,
  ];

  return (
    <div className="guide guide--setup">
      <div className="guide__top">
        <span className="guide__phase">Setup</span>
        <span className="guide__count">
          {assigned}/{s.seats.length} assigned · {bluffs}/3 bluffs · {revealed} revealed
        </span>
      </div>
      ...
      <div className="guide__actions">
        <button className="pbtn" disabled={!canAutoFill} onClick={onAutoFill}>
          🎲 Auto-fill roles
        </button>
        <button className="pbtn" disabled={!allAssigned} onClick={onRevealAll}>
      ...
      <div className="guide__nav">
        <button
          className="pbtn primary-btn"
          disabled={!ready}
          onClick={() => goPhase({ type: "night", count: 1 })}
        >
          Begin Night 1 ▶
        </button>
      </div>
      {!canAutoFill && s.seats.length > 0 && (
        <p className="muted small">Auto-fill needs 5–15 seats (official setup chart).</p>
      )}
